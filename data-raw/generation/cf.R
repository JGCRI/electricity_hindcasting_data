...
generators <- form860processed %>%
  select(year, utility_code, plant_code, generator_code, prime_mover, fuel, summer_capacity)
...
plant.generation.raw <- read.table(paste(base, "data-raw/annual_plant_generation_eia.tsv",sep=''), sep='\t', header=TRUE) %>%
  select(year, plant_code, prime_mover, fuel=fuel_1, net.generation.annual)
